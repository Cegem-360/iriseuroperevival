...
<div class="space-y-4">
    <div class="flex items-center justify-between p-4 bg-linear-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/30 rounded-xl">
        ...
        <div class="text-right">
            <p class="text-2xl font-bold text-blue-400">{{ Number::currency($volunteerPrice, 'EUR') }}</p>
            <p class="text-white/40 text-sm line-through">{{ Number::currency($regularPrice, 'EUR') }}</p>
        </div>
    </div>
    ...
    <div class="flex items-center gap-2 text-green-400 text-sm">
        ...
        <span>You save {{ Number::currency($savings, 'EUR') }} as a volunteer!</span>
    </div>
